...
/**
 * Beschreibt die Parameter einer Funktion und legt die Aufrufmethode
 * 'calculate' fest.
 * 
 * @author tfossi
 * @version 01.08.2014
...
public class FuncPType {
	...
	public final String toString() {
		...
		if (this.values == null)
			rc = "f:=" + this.function + "("
					+ Arrays.asList(this.parameterType)
					+ ") Nur Typen, keine Parameter!";
		else {
			rc = "f:=" + this.function + "( ";
			...
			rc = rc.substring(0, rc.length() - 2)
					+ " ) mit "
					+ (this.function.threePass() ? "Pass3" : (this.function
							.twoPass() ? "Pass2" : "Pass?")) + "-Parameter!";
		}
		return rc;
	}
}
