	...
	final void visibleScreen(Screen screen, Map<String, Widget> widgetsMap) {
		...
		case MI:
			if (basegroup != null && !basegroup.isDisposed()) {
				...
				for (@SuppressWarnings("unused") Control cntrl : basegroup.getChildren()) {
					for (@SuppressWarnings("unused") String set : widgetsMap.keySet()) {
//						if (widgetsMap.get(set).handle == cntrl.handle) {
		...
		case VCM:
			if (basegroup != null && !basegroup.isDisposed()) {
				...
				for (@SuppressWarnings("unused") Control cntrl : basegroup.getChildren()) {
					for (@SuppressWarnings("unused") String set : widgetsMap.keySet()) {
//						if (widgetsMap.get(set).handle == cntrl.handle) {
//							System.out.println("> " + widgetsMap.get(set).handle);
//							cntrl.setVisible(true);
//						}
					}
				}
...
//				}
	 ...
	 * @modified -
	 */
	public SWTGrafikLinux(long threadId) {
